...
export function ZappCard({ project }: ZappCardProps) {
  ...
  return (
    ...
    >
      <div
        className={
          "border-outline-peri bg-surface-card text-text-primary relative flex h-[200px] flex-col justify-between overflow-hidden rounded-xl border p-4 shadow-sm transition-all duration-300 hover:scale-[1.025] hover:shadow-md"
        }
      >
        {/* Category with Icon */}
        <div className="mb-2 flex items-center gap-2">
          <div className="bg-pink/10 rounded-full p-1">{project.icon}</div>
          <span className="text-pink font-display text-xs font-semibold">
            {project.category}
          </span>
        </div>
        {/* Project Name */}
        ...
        {/* Description */}
        <p className="text-text-primary/80 mb-2 line-clamp-2 font-body text-sm">
          {description}
        </p>
        {/* Pills Row */}
        <div className="mt-auto flex flex-wrap gap-2 pt-2">
          {/* Token Price Pill */}
          <span className="bg-mint/20 text-mint flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold">
            <PiCoinVerticalFill className="text-mint text-base" /> Ξ{" "}
            {tokenPrice}
          </span>
          {/* Creator Pill */}
          <span className="bg-pink/20 text-text-primary flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold">
            <FaUser className="text-pink text-sm" />{" "}
            {truncateAddress(project.creator)}
          </span>
          {/* Date Pill */}
          {createdAt && (
            <span className="bg-coin/30 text-text-primary flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold">
              <FaCalendarAlt className="text-coin text-sm" />{" "}
